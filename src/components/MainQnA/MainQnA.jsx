...
import { query, collection, orderBy, doc, Timestamp } from "firebase/firestore";
...
const MainQnA = () => {
  ...
  const [recommendQnA, recommendQnALoad, recommendQnAError] = useCollectionData(
    query(collection(firestore, "QnAs"), orderBy("postViews", "desc")) // 생성일 기준으로 내림차순 정렬
  );
  ...
  return (
    qnaPost && myData && qnaData && recommendQnAData && (
            ...
            >
              {
                recommendQnAData.map((item, index) => (
                  <SwiperSlide key={index} className={styles.slide} onClick={() => showPostModal(item.postID, item.userID)}>
                    <div className={styles.box}>
                      <p className={`${font.fs_16} ${font.fw_7}`}>{item.postTitle}</p>
                    </div>
                    <div className={styles.infoBox}>
                      <div className={styles.leftBox}>
                        <p className={`${font.fs_12} ${font.fw_5} ${font.fc_sub_light}`}>
                          답변 {item.postCmts}
                        </p>
                        <p className={`${font.fs_12} ${font.fw_5} ${font.fc_sub_light}`}>
                          UP {item.postUps}</p>
          ...
          <div className={styles.allPostBox}>
            {
              qnaData.map((item, index) => (
                <div className={styles.postItem} key={index} onClick={() => showPostModal(item.postID, item.userID)}>
                  ...
                  <div className={styles.tagBox}>
                    {
                      item.postTags.map((item, index) => (
                        <p key={index} className={`${font.fs_12} ${font.fw_5} ${styles.tagItem}`}>
                          #{item}
                        </p>
                      ))
                    }
